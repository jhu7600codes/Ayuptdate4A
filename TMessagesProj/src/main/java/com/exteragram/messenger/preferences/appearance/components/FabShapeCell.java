...
public abstract class FabShapeCell extends LinearLayout implements CustomPreferenceCell {
    ...
    public FabShapeCell(Context context) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            final boolean fz = z;
            this.fabShape[i] = new FabShape(context, fz);
            ...
            this.fabShape[i].setOnClickListener(new View.OnClickListener() { // from class: com.exteragram.messenger.preferences.appearance.components.FabShapeCell$$ExternalSyntheticLambda0
                ...
                public final void onClick(View view) {
                    FabShapeCell.this.lambda$new$0(fz, view);
                }
            });
        }
    }
    ...
    static class FabShape extends FrameLayout {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            int i2 = 0;
            while (i2 < 2) {
                int dp3 = dp2 + AndroidUtilities.dp(i2 == 0 ? 0.0f : 32.0f);
                Theme.dialogs_onlineCirclePaint.setColor(PreviewColors.getMockColor(false));
                float f = i * 2;
                canvas.drawRoundRect(dp - i, dp3 - i, dp + i, dp3 + i, ExteraConfig.getAvatarCorners(f, true), ExteraConfig.getAvatarCorners(f, true), Theme.dialogs_onlineCirclePaint);
                int i3 = 0;
                while (i3 < 2) {
                    Theme.dialogs_onlineCirclePaint.setColor(PreviewColors.getMockColor(i3 == 0));
                    int i4 = i3 * 10;
                    this.rect.set(AndroidUtilities.dp(41.0f), dp3 - AndroidUtilities.dp(7 - i4), getMeasuredWidth() - AndroidUtilities.dp(i3 == 0 ? 70.0f : 55.0f), dp3 - AndroidUtilities.dp(3 - i4));
