...
public abstract class BasePreferencesActivity extends BaseFragment {
    ...
    public View createView(Context context) {
        ...
        UniversalRecyclerView universalRecyclerView = new UniversalRecyclerView(this, new Utilities.Callback2() { // from class: com.exteragram.messenger.preferences.BasePreferencesActivity$$ExternalSyntheticLambda6
            ...
            public final void run(Object obj, Object obj2) {
                ArrayList<UItem> items = (ArrayList<UItem>) obj;
                BasePreferencesActivity.this.fillItems(items, (UniversalAdapter) obj2);
                for (int k = 0; k < items.size(); k++) {
                    final UItem item = items.get(k);
                    if (item != null && item.viewType == UniversalAdapter.VIEW_TYPE_EXPANDABLE_SWITCH && item.clickCallback == null) {
                        final int position = k;
                        item.clickCallback = v -> BasePreferencesActivity.this.onClick(item, v, position, 0, 0);
                    }
                }
            }
            ...
            public final void run(Object obj, Object obj2, Object obj3, Object obj4, Object obj5) {
                ...
                View view = (View) obj2;
                float x = ((Float) obj4).floatValue();
                if (item != null && view != null && item.viewType == UniversalAdapter.VIEW_TYPE_EXPANDABLE_SWITCH && item.clickCallback2 != null) {
                    boolean onSwitch = LocaleController.isRTL ? x < AndroidUtilities.dp(76) : x > view.getMeasuredWidth() - AndroidUtilities.dp(76);
                    if (onSwitch) {
                        item.clickCallback2.onClick(view);
                        return;
                    }
                }
                BasePreferencesActivity.this.onClick(item, view, ((Integer) obj3).intValue(), x, ((Float) obj5).floatValue());
            }
    ...
    public void scrollToItem(final int i) {
        ...
        int findPositionByItemId = universalRecyclerView.findPositionByItemId(i);
        if (findPositionByItemId >= 0 && findPositionByItemId < this.listView.adapter.getItemCount()) {
            this.layoutManager.scrollToPositionWithOffset(findPositionByItemId, AndroidUtilities.dp(80.0f));
            this.listView.highlightRow(new RecyclerListView.IntReturnCallback() { // from class: com.exteragram.messenger.preferences.BasePreferencesActivity$$ExternalSyntheticLambda2
                ...
                public final int run() {
                    int lambda$scrollToItem$0;
                    lambda$scrollToItem$0 = BasePreferencesActivity.this.lambda$scrollToItem$0(i);
                    return lambda$scrollToItem$0;
                }
            });
        } else {
    ...
    public /* synthetic */ void lambda$showListDialog$1(boolean z, int i, PopupUtils.OnItemClickListener onItemClickListener, UItem uItem, CharSequence[] charSequenceArr, int i2) {
        ...
        View findViewByItemId = this.listView.findViewByItemId(uItem.id);
        if (findViewByItemId instanceof TextCell) {
            ((TextCell) findViewByItemId).setValue(charSequenceArr[i2], true);
        }
    ...
    protected void toggleBooleanSettingAndRefresh(SharedPreferences sharedPreferences, String str, UItem uItem, Consumer consumer) {
        ...
        uItem.setChecked(z);
        View findViewByItemId = this.listView.findViewByItemId(uItem.id);
        if (findViewByItemId instanceof CheckBoxCell) {
            ((CheckBoxCell) findViewByItemId).setChecked(z, true);
        } else if (findViewByItemId instanceof TextCheckCell) {
            ((TextCheckCell) findViewByItemId).setChecked(z);
        }
        this.listView.adapter.update(true);
    }
                ...
                if (z) {
                    if (longSparseArray != null && longSparseArray.size() == 1) {
                        ...
                        if (j == 0 || j == BasePreferencesActivity.this.getUserConfig().getClientUserId()) {
                            formatString = LocaleController.getString(R.string.SettingLinkToSavedMessages);
                        } else {
                            formatString = LocaleController.formatString(R.string.SettingLinkToUser, BasePreferencesActivity.this.getMessagesController().getPeerName(j, true));
                        }
                    } else {
                        formatString = LocaleController.formatString(R.string.SettingLinkToChats, LocaleController.formatPluralString("Chats", i, new Object[0]));
                    }
                    Bulletin createSimpleBulletin = BulletinFactory.of(BasePreferencesActivity.this).createSimpleBulletin(R.raw.forward, formatString);
