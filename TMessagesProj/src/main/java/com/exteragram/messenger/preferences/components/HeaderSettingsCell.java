...
public class HeaderSettingsCell extends LinearLayout implements CustomPreferenceCell {
    ...
    public HeaderSettingsCell(Context context) {
        ...
        setOrientation(1);
        setGravity(17);
        Drawable mutate = ContextCompat.getDrawable(context, R.drawable.ic_foreground_extera).mutate();
        Theme.ThemeInfo activeTheme = Theme.getActiveTheme();
        int color = ContextCompat.getColor(context, R.color.ic_background_extera);
        if (activeTheme.isMonet() && Build.VERSION.SDK_INT >= 31) {
            color = MonetUtils.getColor(activeTheme.isDark() ? "a2_800" : "a1_100");
            mutate = ContextCompat.getDrawable(context, R.drawable.ic_foreground_solid).mutate();
            mutate.setColorFilter(new PorterDuffColorFilter(MonetUtils.getColor(activeTheme.isDark() ? "a1_200" : "a1_700"), PorterDuff.Mode.MULTIPLY));
        }
        ...
        rLottieImageView.setImageDrawable(mutate);
    ...
    public void setIsAyuGram(boolean z) {
        ...
        if (z) {
            ...
            this.subtitleTextView.setText(org.telegram.messenger.BuildVars.BUILD_VERSION_STRING);
            Theme.ThemeInfo activeTheme = Theme.getActiveTheme();
            if (activeTheme.isMonet() && Build.VERSION.SDK_INT >= 31) {
                mutate = ContextCompat.getDrawable(getContext(), R.drawable.ic_logo_foreground).mutate();
                mutate.setColorFilter(new PorterDuffColorFilter(MonetUtils.getColor(activeTheme.isDark() ? "a1_200" : "a1_700"), PorterDuff.Mode.MULTIPLY));
                this.imageView.setBackgroundColor(MonetUtils.getColor(activeTheme.isDark() ? "a2_800" : "a1_100"));
            } else {
                mutate = ContextCompat.getDrawable(getContext(), R.drawable.ic_logo_foreground2).mutate();
                this.imageView.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.ic_background_settings).mutate());
            }
