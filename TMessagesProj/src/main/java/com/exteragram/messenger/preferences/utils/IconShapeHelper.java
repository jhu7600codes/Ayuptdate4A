...
public final class IconShapeHelper {
    ...
    public final Path getFinalIconShapePath(float f, float f2, float f3) {
        ...
        float dpf2 = AndroidUtilities.dpf2(f);
        float dpf22 = AndroidUtilities.dpf2(f2);
        float dpf23 = AndroidUtilities.dpf2(f3);
        if (dpf23 > 0.0f && z && isSystemPathSquare) {
            Path path = new Path();
            RectF rectF = scratchRect;
            rectF.set(0.0f, 0.0f, dpf2, dpf22);
            path.addRoundRect(rectF, dpf23, dpf23, Path.Direction.CW);
            return path;
        }
        return resizePath(defaultPath, dpf2, dpf22);
    }
    ...
    private final Path getDefaultPath() {
        Path createPathFromPathData = PathParser.createPathFromPathData("M50,0A50,50,0,0,1,50,100A50,50,0,0,1,50,0");
    ...
    private final boolean hasSharpCorners(Path path, RectF rectF) {
        ...
        int max = Math.max(3, (int) (Math.min(rectF.width(), rectF.height()) * 0.1f));
        Region region = new Region();
        region.setPath(path, new Region((int) rectF.left, (int) rectF.top, (int) rectF.right, (int) rectF.bottom));
        float f = rectF.left;
        float f2 = rectF.top;
        Rect rect = new Rect((int) f, (int) f2, ((int) f) + max, ((int) f2) + max);
        float f3 = rectF.right;
        float f4 = rectF.top;
        Rect rect2 = new Rect(((int) f3) - max, (int) f4, (int) f3, ((int) f4) + max);
        float f5 = rectF.right;
        float f6 = rectF.bottom;
        Rect rect3 = new Rect(((int) f5) - max, ((int) f6) - max, (int) f5, (int) f6);
        float f7 = rectF.left;
        float f8 = rectF.bottom;
        Rect[] rectArr = {rect, rect2, rect3, new Rect((int) f7, ((int) f8) - max, ((int) f7) + max, (int) f8)};
        Region region2 = new Region();
        for (int i = 0; i < 4; i++) {
